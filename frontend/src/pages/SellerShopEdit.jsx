...
function ShopSectionsEditor({ form, setForm }) {
  ...
  return (
    <div className="space-y-2">
      ...
      {sections.map((s, i) => (
        <div key={s.id} className="space-y-1" data-testid={`shop-section-row-${i}`}>
          ...
          <label className="flex items-center gap-2 text-[11px] text-[var(--js-text-secondary)] pl-8">
            <input
              ...
              data-testid={`shop-section-promo-toggle-${i}`}
            />
            <span>Auto-populate with products on <strong>live promo</strong> (hidden when empty).</span>
          </label>
        </div>
      ))}
      <button
