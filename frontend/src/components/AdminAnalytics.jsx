...
export default function AdminAnalytics() {
  ...
  if (loading) {
    return <div className="py-12 text-center text-sm text-[var(--js-text-secondary)]">{tr("loadingAnalytics")}</div>;
  }
  if (!data) {
    return <div className="py-12 text-center text-sm text-[var(--js-text-secondary)]">{tr("couldNotLoadAnalytics")}</div>;
  }
  ...
  return (
    <div className="space-y-8" data-testid="admin-analytics">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <Stat icon={ShoppingBag} label={tr("totalOrders")} value={t.orders ?? 0} color="#C84B31" />
        <Stat icon={DollarSign} label={tr("totalRevenue")} value={formatUSD(t.revenue_usd ?? 0)} color="#2D6A4F" />
        <Stat icon={Clock} label={tr("pendingOrders")} value={t.pending_orders ?? 0} color="#E9C46A" />
        <Stat icon={TrendingUp} label={tr("deliveredOrders")} value={t.delivered_orders ?? 0} color="#0E1A2B" />
        <Stat icon={Users} label={tr("customers")} value={t.customers ?? 0} color="#4E598C" />
        <Stat icon={Store} label={tr("sellers")} value={t.sellers ?? 0} color="#8B5CF6" />
        <Stat icon={Store} label={tr("activeShops")} value={t.shops ?? 0} color="#C84B31" />
        <Stat icon={ShoppingBag} label={tr("products")} value={t.products ?? 0} color="#0E1A2B" />
      </div>

      {/* Charts row */}
      <div className="grid md:grid-cols-2 gap-6">
        <div className="bg-white border border-[var(--js-border)] rounded-2xl p-5">
          <p className="font-display font-bold text-[var(--js-text)] mb-4">{tr("ordersLast30Days")}</p>
          <div className="h-64">
        ...
        <div className="bg-white border border-[var(--js-border)] rounded-2xl p-5">
          <p className="font-display font-bold text-[var(--js-text)] mb-4">{tr("newUsersLast30Days")}</p>
          <div className="h-64">
            <ResponsiveContainer width="100%" height="100%">
      ...
      {/* Top sellers */}
      <div className="bg-white border border-[var(--js-border)] rounded-2xl p-5">
        <p className="font-display font-bold text-[var(--js-text)] mb-4">{tr("topSellers")}</p>
        {(data.top_sellers || []).length === 0 ? (
          <p className="text-sm text-[var(--js-text-secondary)]">{tr("noSalesYet")}</p>
        ) : (
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="text-left text-[10px] uppercase tracking-wider text-[var(--js-text-secondary)] border-b border-[var(--js-border)]">
                  ...
                  <th className="py-2 pr-2 font-semibold">{tr("seller")}</th>
                  <th className="py-2 pr-2 font-semibold text-right">{tr("revenue")}</th>
                </tr>
              </thead>
              <tbody>
